...
# @csrf_exempt  # فقط برای تست، در محیط تولید از CSRF استفاده کنید
# def ppt_to_pdf(request):
#     if request.method == 'POST' and request.FILES.get('ppt_file'):
#         ppt_file = request.FILES['ppt_file']
#
...
#         # مسیر فایل PDF خروجی
#         pdf_filename = f"converted_{int(time.time())}.pdf"
#         pdf_dir = os.path.join(settings.MEDIA_ROOT, 'temp')
#         pdf_path = os.path.join(pdf_dir, pdf_filename)
#
...
#
#         # مقداردهی اولیه COM
#         pythoncom.CoInitialize()  # اگر از win32com استفاده می‌کنید
#
#         try:
#             # تبدیل PowerPoint به PDF با استفاده از Microsoft PowerPoint
#             powerpoint = comtypes.client.CreateObject("Powerpoint.Application")
...
#             powerpoint.Quit()
#         except Exception as e:
#             # در صورت بروز خطا، فایل موقت را پاک کنید
#             os.unlink(temp_ppt.name)
#             return JsonResponse({'error': str(e)}, status=500)
#         finally:
#             # آزادسازی COM
#             pythoncom.CoUninitialize()  # اگر از win32com استفاده می‌کنید
#
#         # پاک کردن فایل موقت PowerPoint
#         os.unlink(temp_ppt.name)
#
